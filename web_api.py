...
async def get_machine_status(machine_name: str):
    ...
    try:
        ...
        return create_response(True, "获取状态成功", {
            ...
            'last_communication': stats['last_communication'],
...
@app.post("/api/machines/{machine_name}/storage/start", response_model=ApiResponse)
async def start_storage_process(machine_name: str, request: Request):
    """开始存件流程"""
    try:
        ...
        return create_response(True, "存件流程启动成功，请等待无人机降落")
    
    except Exception as e:
        logger.error(f"启动存件流程失败: {e}")
        return create_response(False, f"启动存件流程失败: {str(e)}")
...
async def confirm_drone_takeoff(machine_name: str):
    ...
    try:
        ...
        for _ in range(10):
            ...
            if status == 21:
                # 关闭舱门
                door_result = door_controller.close_door(1, client)
...
async def operate_door(request: DoorOperationRequest):
    ...
    try:
        ...
        if request.operation.lower() == "open":
            success = door_controller.open_door(request.position, connection)
            ...
            success = door_controller.close_door(request.position, connection)
            machine_info = f"[{request.machine_name}]" if request.machine_name else ""
